...
class PrototypicalReadout:
    ...
    def decision_function(self, features: np.ndarray) -> np.ndarray:
        ...
        Higher score means closer to class prototype.
        """
        assert self.prototypes is not None, "Call .fit() before .decision_function()"
        diff = features[:, np.newaxis, :] - self.prototypes[np.newaxis, :, :]
...
def _save_confusion_matrix_artifacts(
    ...
    cm_csv_path = output_dir / f"{method_name}_{split_name}_confusion_matrix.csv"
    ...
    plt.tight_layout()
    cm_png_path = output_dir / f"{method_name}_{split_name}_confusion_matrix.png"
...
def _save_roc_auc_artifacts(
    ...
    """
    Save ROC curve plot + per-class curve points and AUC summary.
    """
    import pandas as pd

    y_true_bin = label_binarize(y_true, classes=labels_eval)
    if y_true_bin.ndim == 1:
        y_true_bin = y_true_bin.reshape(-1, 1)

    # Align score columns with ordered class labels.
    if y_scores.shape[1] != len(labels_eval):
        raise ValueError(
            f"y_scores has {y_scores.shape[1]} columns but expected {len(labels_eval)} classes."
        )
    ...
    for idx, cls in enumerate(labels_eval):
        y_cls = y_true_bin[:, idx]
        # Skip ROC for absent class in this split.
        if len(np.unique(y_cls)) < 2:
            auc_rows.append({"class": int(cls), "auc": np.nan})
            continue

        fpr, tpr, thresholds = roc_curve(y_cls, y_scores[:, idx])
        cls_auc = auc(fpr, tpr)
        auc_rows.append({"class": int(cls), "auc": float(cls_auc)})

        for fpr_v, tpr_v, thr_v in zip(fpr, tpr, thresholds):
            roc_rows.append(
                {
                    "class": int(cls),
                    "fpr": float(fpr_v),
        ...
        ax.plot(fpr, tpr, lw=2, label=f"Class {cls} (AUC={cls_auc:.3f})")

    # Micro and macro AUC summaries when computable.
    micro_auc = np.nan
    macro_auc = np.nan
    try:
        micro_auc = float(roc_auc_score(y_true_bin, y_scores, average="micro", multi_class="ovr"))
        macro_auc = float(roc_auc_score(y_true_bin, y_scores, average="macro", multi_class="ovr"))
    except ValueError:
        pass

    auc_rows.append({"class": "micro", "auc": micro_auc})
    auc_rows.append({"class": "macro", "auc": macro_auc})

    ax.plot([0, 1], [0, 1], "k--", lw=1, label="Chance")
    ax.set_xlabel("False Positive Rate")
    ax.set_ylabel("True Positive Rate")
    ax.set_title(f"ROC Curve - {method_name} - {split_name}")
    ax.legend(loc="lower right", fontsize=8)
    ax.grid(alpha=0.3)
    plt.tight_layout()

    roc_png_path = output_dir / f"{method_name}_{split_name}_roc_curve.png"
    plt.savefig(roc_png_path, dpi=200)
    plt.close(fig)

    roc_csv_path = output_dir / f"{method_name}_{split_name}_roc_points.csv"
    auc_csv_path = output_dir / f"{method_name}_{split_name}_auc_summary.csv"
    pd.DataFrame(roc_rows).to_csv(roc_csv_path, index=False)
    pd.DataFrame(auc_rows).to_csv(auc_csv_path, index=False)

    print(f"Saved ROC curve image to: {roc_png_path}")
    print(f"Saved ROC points CSV to: {roc_csv_path}")
    print(f"Saved AUC summary CSV to: {auc_csv_path}")
...
def _evaluate_and_save_artifacts(
    ...
    report = classification_report(
        ...
        y_pred,
        labels=labels_eval,
        target_names=[str(i) for i in labels_eval],
        output_dict=True,
        zero_division=0,
    )
    report_df = pd.DataFrame(report).transpose()
    report_csv_path = output_dir / f"{method_name}_{split_name}_classification_report.csv"
    report_df.to_csv(report_csv_path)
    print(f"Saved classification report to: {report_csv_path}")

    _save_confusion_matrix_artifacts(
        y_true=y_true,
        y_pred=y_pred,
...
def main(
    ...
    labels_eval = sorted(np.unique(np.concatenate([y_train, y_test])).tolist())
    _evaluate_and_save_artifacts(
        method_name="balanced_svc",
        split_name="test",
        y_true=y_test,
        y_pred=y_pred_svc,
        y_scores=clf.decision_function(F_test),
        labels_eval=labels_eval,
        output_dir=output_dir,
    )
    ...
    y_scores_proto = proto_full.decision_function(F_test)
    ...
    print(classification_report(y_test, y_pred_proto,
                                 ...
                                 zero_division=0))
    _evaluate_and_save_artifacts(
        method_name="proto_full",
        split_name="test",
        y_true=y_test,
        y_pred=y_pred_proto,
        y_scores=y_scores_proto,
        labels_eval=labels_eval,
        output_dir=output_dir,
    )
    ...
    y_scores_kshot = proto_kshot.decision_function(F_test)
    ...
    print(classification_report(y_test, y_pred_kshot,
                                 ...
                                 zero_division=0))
    _evaluate_and_save_artifacts(
        method_name=f"proto_{k_shot}shot",
        split_name="test",
        y_true=y_test,
        y_pred=y_pred_kshot,
        y_scores=y_scores_kshot,
        labels_eval=labels_eval,
        output_dir=output_dir,
    )
